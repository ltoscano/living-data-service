...
const LivingDataService = () => {
  ...
  // Custom hooks
  const { isAuthenticated, user, isLoading, login, logout, changePassword } = useAuth();
  const { 
    modal, 
    showSuccess, 
    showError, 
  ...
  const handleLogout = async () => {
    try {
      // Controlla se Keycloak è abilitato dall'endpoint config
      const configResponse = await fetch('/api/auth/config');
      const configData = await configResponse.json();
      
      console.log('🔐 Logout - Config data:', configData);
      
      // Se Keycloak è abilitato (indipendentemente dallo stato di autenticazione)
      if (configData.keycloakEnabled) {
        console.log('🔐 Force logout: destroying all sessions (Keycloak + Local)');
        window.location.href = '/auth/keycloak/logout';
      } else {
        console.log('🔐 Using local logout only');
        await logout('local');
      }
    } catch (error) {
      console.error('Logout error:', error);
      // Fallback: prova comunque il logout Keycloak se l'endpoint config non funziona
      console.log('🔐 Fallback: trying Keycloak logout');
      window.location.href = '/auth/keycloak/logout';
    }
  };
